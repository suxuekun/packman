...
void Board2:: move(char command)
{
	if ('p'!=command)
	{
	  switch (command)
	   { case 'w': if (player_id_x<=0) break;
			else
			  { 
		     ...
		     break;
	     case 's': if (player_id_x>=the_rows-1) break;
			else
			  { 
                            if (!  mySpace[player_id_x+1][player_id_y]->isThereAWall())
			      { mySpace[player_id_x][player_id_y]->removeOnePlayer();
	   ...
	   if (cookieValue>0)
		{
		    std::cerr << "event : Ate a cookie\n";
		    currentScore+=cookieValue;
		}
        if (mySpace[player_id_x][player_id_y]->isThereAMonster())
        {
            std::cerr << "event : meet a monster\n";
        }
        if (mySpace[player_id_x][player_id_y]->isThereABomb())
        {
            std::cerr << "event : meet a bomb\n";
        }
